...
source("./weight.R")       # Weighting and raking
source("./fig_setup.R")    # Figure specifications
source("./ad_subg_tab.R")  # Sub-group function for making figures
source("./nb_summary.R")   # Net benefit function for making summary scale figures
source("./nb_details.R")   # Net benefit function for making detail component figures
...
nb_profile <- bind_rows(value, life, career, skill, affinity)
...
t_nb_profile <- nb_profile %>% mutate(grp = "Total")

# SOC
value = nb_value(subset(sch.w, soc == "Not White"))
life = nb_life(subset(sch.w, soc == "Not White"))
career = nb_career(subset(sch.w, soc == "Not White"))
skill = nb_skill(subset(sch.w, soc == "Not White"))
affinity = nb_affinity(subset(sch.w, soc == "Not White"))

soc_nb_profile <- bind_rows(value, life, career, skill, affinity) %>% 
  mutate(grp = "Students of color")

# First gen
value = nb_value(subset(sch.w, fgen == "Yes"))
life = nb_life(subset(sch.w, fgen == "Yes"))
career = nb_career(subset(sch.w, fgen == "Yes"))
skill = nb_skill(subset(sch.w, fgen == "Yes"))
affinity = nb_affinity(subset(sch.w, fgen == "Yes"))

fgen_nb_profile <- bind_rows(value, life, career, skill, affinity) %>% 
  mutate(grp = "First generation")

# Female
value = nb_value(subset(sch.w, q274 == "Woman"))
life = nb_life(subset(sch.w, q274 == "Woman"))
career = nb_career(subset(sch.w, q274 == "Woman"))
skill = nb_skill(subset(sch.w, q274 == "Woman"))
affinity = nb_affinity(subset(sch.w, q274 == "Woman"))

fem_nb_profile <- bind_rows(value, life, career, skill, affinity) %>% 
  mutate(grp = "Female")

# Non-traditional
value = nb_value(subset(sch.w, ntrad == "Yes"))
life = nb_life(subset(sch.w, ntrad == "Yes"))
career = nb_career(subset(sch.w, ntrad == "Yes"))
skill = nb_skill(subset(sch.w, ntrad == "Yes"))
affinity = nb_affinity(subset(sch.w, ntrad == "Yes"))

ntrad_nb_profile <- bind_rows(value, life, career, skill, affinity) %>% 
  mutate(grp = "Non-Traditional")

# Recent
value = nb_value(subset(sch.w, recg == "Yes"))
life = nb_life(subset(sch.w, recg == "Yes"))
career = nb_career(subset(sch.w, recg == "Yes"))
skill = nb_skill(subset(sch.w, recg == "Yes"))
affinity = nb_affinity(subset(sch.w, recg == "Yes"))

recg_nb_profile <- bind_rows(value, life, career, skill, affinity) %>% 
  mutate(grp = "Recent graduate")

subg_nb_profile <- bind_rows(t_nb_profile, 
                             soc_nb_profile, 
                             fgen_nb_profile, 
                             fem_nb_profile,
                             ntrad_nb_profile,
...
ggplot(subg_nb_profile, aes(grp, nb, label=round(nb*100, 0), fill=grp)) +
    geom_col() +
    geom_text(
      color = "white",
      hjust = 1.3,
...
**Question:** Are we doing this?


# Detailed findings

**Note:** Switched to horizontal layout as suggested

## Net benefit profile - Value

```{r}

value <- dnb_value(sch.w) 

value$dummy <- 1
value$pos <- 1.1

d1 <- dict %>% 
  select(var = uvar, label) %>% unique()

value <- left_join(value, d1)
...
v1$var22 <- case_when(
  v1$var2 == "val1" ~ "Strongly negative",
  v1$var2 == "val2" ~ "Somewhat negative",
  v1$var2 == "val3" ~ "Neither positive nor negative",
  v1$var2 == "val4" ~ "Somewhat positive",
  v1$var2 == "val5" ~ "Strongly positive",
)

v1$var22 <- factor(v1$var22, 
                    levels = c(
                       "Strongly positive",
                       "Somewhat positive",
...
affinity <- left_join(affinity, d1)
